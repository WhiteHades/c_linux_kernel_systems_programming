...
void read_cards(void) {
  ...
  char input[20];
  int cards_read = 0;
  for (int rank = 0; rank < NUM_RANKS; rank++) num_in_rank[rank] = 0;
  for (int suit = 0; suit < NUM_SUITS; suit++) num_in_suit[suit] = 0;
...
void analyze_hand(void) {
  ...
  if (num_in_rank[12] == 1 && num_in_rank[0] == 1 &&
      num_in_rank[1] == 1 && num_in_rank[2] == 1 && num_in_rank[3] == 1)
    straight = true;
